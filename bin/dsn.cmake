...
macro(dsn_setup_serialization)
    ...
    if (MY_PROJ_LANG STREQUAL "CXX")
        if (USE_THRIFT)
            if (DSN_BUILD_RUNTIME)
                list(APPEND MY_PROJ_INC_PATH ${THRIFT_INCLUDE_DIR})
                list(APPEND MY_PROJ_LIB_PATH ${THRIFT_LIB_DIR})
            endif ()
            if (WIN32)
                list(APPEND MY_PROJ_LIBS thriftmt)
            else ()
                list(APPEND MY_PROJ_LIBS thrift.so)
            endif ()
        ...
        if (USE_PROTOBUF)
            if (DSN_BUILD_RUNTIME)
                list(APPEND MY_PROJ_INC_PATH ${PROTOBUF_INCLUDE_DIR})
                list(APPEND MY_PROJ_LIB_PATH ${PROTOBUF_LIB_DIR})
            endif ()
            if (WIN32)
    ...
    if(MY_PROJ_LANG STREQUAL "CXX")
        ...
        if((MY_PROJ_TYPE STREQUAL "SHARED") OR (MY_PROJ_TYPE STREQUAL "EXECUTABLE"))
            if(DSN_BUILD_RUNTIME AND(DEFINED DSN_IN_CORE) AND DSN_IN_CORE)
                ...
                set(TEMP_LIBS dsn.dev.cpp dsn.core dsn.dev.utility)
            endif()
